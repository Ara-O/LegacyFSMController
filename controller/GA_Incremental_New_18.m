...
% The flipping is done at the offspring and it doesn't generate a new
% offspring. If the difference between the best plans is smaller than one,
% it does the flipping only at one position. If it is bigger than one, it
% does the flipping at 3 random points.
%The mutation is applied at the parents creating new offpring. It only
%happens when the change_value is a mutliple of 5 or the same_result is
%bigger than 10. If the same_result is bigger than 30, it tries to create
%totally new paths based on the start events of the chosen mutation parents 
function [all_plan,plan,plan_state,plan_cost,num_spoint,initial_plan,num_of_changes,idx_change,trac_plan_cost,num_rep,t_dur,t_pop] = GA_Incremental_New_18(population_size,max_rep,mutation_rate,stop_cond,states,start_loc,numTasks,numRobots,numRegions,tasks,regions,robot_tasks,states_incr,events_incr,initial_state_incr,initial_state,events)
num_of_changes=0;
abstracted = false;
...
while num_rep<max_rep && aux_same_result<stop_cond
    ...
    for i=1:2:(length(couples)-1)
        ...
        if ~isempty(jindex) %check if there is a shared state
            ...
            if (change_value/12)<1 && aux_same_result<10
                ...
                if change_value >=1
                    if diff_plan(change_value)>1
                        ...
                        [offspring{ind},offspring_event{ind}] = newCrossover(1,offspring_event{ind},states_incr,num_fsm,initial_state_incr,event_list,events_incr);
                    end
                end
            else
                if aux_same_result>5
                    ...
                    [offspring{ind},offspring_event{ind}] = newCrossover(1,parent_1_events,states_incr,num_fsm,initial_state_incr,event_list,events_incr);
            ...
            if (change_value/12)<1 && aux_same_result<10
               ...
               if aux_same_result>5
                ...
                [offspring{ind},offspring_event{ind}] = newCrossover(1,parent_2_events,states_incr,num_fsm,initial_state_incr,event_list,events_incr);
               end
            end
    ...
    mutation_idx_max=ceil(length(couples)*mutation_rate);
    %it randomly distributes the indexes of offspring
    mutation_idx_total=randperm(length(couples));
    ...
    if aux_same_result > 30
        for i=1:length(mutation_idx)
            old_offspring=stripEventPathMod(path_events{couples(mutation_idx(i))});
            ...
            [offspring{ind},offspring_event{ind}] = createChromossome(path_j,states_incr,num_fsm,initial_state_incr,event_list,events_incr);
            ind=ind+1;
        ...
        for i=1:length(mutation_idx)
           cost_extra_aux_off=[];
            ...
            [cost_aux_off,cost_trans_aux_off,cost_extra_aux_off]= path_cost_Incr(path_gen{couples(mutation_idx(i))},path_events{couples(mutation_idx(i))},start_loc,states,tasks,regions,numRegions,numTasks,abstracted,start_events_only,robot_tasks,numRobots);
            
            
            
            mut_off='';
            mut_off=path_gen{couples(mutation_idx(i))};
            %offspring{mutation_idx(i)}='';
            mut_off_events='';
            mut_off_events=path_events{couples(mutation_idx(i))};%offspring_event{mutation_idx(i)};
            %offspring_event{mutation_idx(i)}='';
            ...
            if ~isempty(potential_nodes)
                ...
                idx_mut1=pot_nodes(idx_pot);
                
            else
                idx_mut1=randi([2 (length(mut_off)-1)]);
            end
            ...
            offspring{ind}={mut_off{1,1:idx_mut-1}}; %it saves the plan til the previous states
            kj=idx_mut;
            offspring_event{ind}={mut_off_events{1:idx_mut-2}};
            ...
            while ~detectEndState(cur_states,states_incr,num_fsm)
                ...
                offspring_event{ind}{kj-1}=detect_event;
                cur_states = updateCurEvent(detect_event,cur_states,events_incr,states_incr,num_fsm);
                index=findSameState(offspring{ind},cur_states,num_fsm);
                if isempty(index)
                    offspring{ind}{kj} =cur_states;%saves this state, which will be verified to see if it is the marked state at the next run
                    kj=kj+1;
                else
                    old_path_single='';
                    old_path_single=offspring{ind};
                    offspring{ind}='';
                    old_event_single='';
                    old_event_single=offspring_event{ind};
                    path_events{j}='';
                    offspring{ind}={old_path_single{1,1:index}};
                    cur_states=old_path_single{1,index};
                    offspring_event{ind}={old_event_single{1:index-1}};
                    kj=index+1;
                end
