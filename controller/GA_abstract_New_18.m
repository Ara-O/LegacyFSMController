 function [all_plan,plan,plan_state,plan_cost,num_spoint,initial_plan,num_of_changes,idx_change,trac_plan_cost,num_rep,t_dur,t_pop] = GA_abstract_New_18(population_size,max_rep,mutation_rate,stop_cond,states,start_loc,numTasks,numRobots,numRegions,tasks,regions,robot_tasks)
num_of_changes=0;
abstracted = false;
...
plan={{'a05s'},{'b01s'},{'b09e'},{'a14e'},{'a13e'},{'b01f'},{'b02s'},{'a05f'},{'a06s'},{'b08e'},{'b02f'},{'b03s'},{'a14e'},{'b07e'},{'a15e'}, {'b03f'},{'b04s'},{'a06f'},{'b12e'},{'b04f'}};

sz_plan=length(plan);
act_state=1;
i=1;
for k=1:sz_plan+1
        path_gen{1}(i)=act_state; %save at the state being analyzed
        ...
        for r=1:sz
            if strcmpi(states{act_state,2}(r,1),plan{k}{1})
...
path_events='';
id=1;
for ik=1:length(old_path_gen)
    b=length(old_path_gen);
    auxk=1;
    for il=1:b
        %sum(old_path_gen{ik}==path_gen{il})
        if il ~= ik
            if length(old_path_gen{ik})==length(old_path_gen{il})
                if sum(old_path_gen{ik}==old_path_gen{il})>=length(old_path_gen{ik})
                    auxk=0;
                    % pause;
                    break;
                end
    ...
    if auxk
        path_gen{id}=old_path_gen{ik};
        path_events{id}=old_event_gen{ik};
        id=id+1;
    end
end
% for ik=1:length(old_path_gen)
%     path_gen{ik}=old_path_gen{ik};
%     path_events{ik}=old_event_gen{ik};
% end
...
%     num_spoint(num_rep)=0;
   ...
   id=1;
 for ik=1:length(aux_old_gen)
    b=length(old_path);
    auxk=1;
    for il=1:b
        %sum(old_path_gen{ik}==path_gen{il})
        %if il ~= ik
            if length(aux_old_gen{ik})==length(old_path{il})
                if sum(aux_old_gen{ik}==old_path{il})>=length(aux_old_gen{ik})
                    auxk=0;
                    % pause;
                    break;
                end
