function [path,path_events]= createChromosomeRegionsHorizon3(path_abs,states_incr,num_fsm,initial_state_incr,event_list,events_incr,numRobots,numTasks,taskRegions,initialRegions,numRegions)
i=1;
state_i=1;
event_i=1;
index=1;
...
for k=1:length(path_abs{1})
    if strcmp(path_abs{1}{k}{1}(end),'f')
        aux_f=1;
        finish_event{index}=path_abs{1}{k}{1};
        index=index+1;
        task=str2num(path_abs{1}{k}{1}(2:end-1));
        if strcmp(path_abs{1}{k}{1}(1),'a')
            desired_region1(end+1)=taskRegions(task);
        elseif strcmp(path_abs{1}{k}{1}(1),'b')
        ...
        [aux_event]=checkEventPath(possible_transitions,path_abs,k);
        idx_pot=aux_event;
        detect_event=possible_transitions{idx_pot,1}; %save the selected the transition/event
%         i=i+1;
        %j=j+1;
        finish_event{index}=strcat(path_abs{1}{k}{1}(1:end-1),'f');
        index=index+1;
...
if k-aux_finish==1
    ...
    if only_finish==1
        path_events={path_event_middle{indPlan}{1:end}};
        path={path_state_middle{indPlan}{1:end}};
    else
        path_events={path_events{1:end},path_event_middle{indPlan}{1:end}};
        path={path{1:end},path_state_middle{indPlan}{1:end}};
    end
