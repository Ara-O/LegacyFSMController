function [transition_cost,trans_event,new_X,new_Y] ...
          = calcTransitionCosTasks(next_event,cur_state,state_path,event_path,states,tasks,EEC,regions,numRegions,start_loc,...
...
if ~abstracted && ~start_events_only % && regions_exist
  ...
  if strcmp(regNum,'11')
    ...
    if(strcmp(bot_task,''))
        %if bot_tasks(cur_state,taskRobot)==0

            [taskX,taskY] = calcRegCenter(regNum,numRegions,numTasks,regions);
%         else
%                   %if(local)
%             bot_task = bot_tasks(cur_state,taskRobot);
