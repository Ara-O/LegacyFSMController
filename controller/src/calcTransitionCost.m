function [transition_cost,trans_event,new_X,new_Y] ...
          = calcTransitionCost(next_event,cur_state,state_path,states,tasks,EEC,regions,numRegions,start_loc,...
...
if ~abstracted && ~start_events_only % && regions_exist
  ...
  if strcmp(regNum,'11')
    ...
    %elseif numRegions-numTasks == 4 % if 4 regions,
      ...
      [inter_x,inter_y] = intersections(x_coordsA,y_coordsA,x_coordsB,y_coordsB);
      aux_inter=0;  
...
%     if aux_inter
%         transition_cost=transition_cost+20;
%     end
    new_X = inter_x;
    new_Y = inter_y;
