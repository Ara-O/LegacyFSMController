...
% %     cluster_mask=convertToMask(cluster);
% %     cluster_mask1=mutationAbs(cluster_mask);
%     path_abs(i) = {{'b05s'},{'c01s'},{'a03s'},{'b06s'},{'a04s'},{'c07s'},{'c08s'},{'c02s'}};%convertToEvent(cluster_mask1,events,numTasks);
% end
num_paths=population_size;%length(path_abs);
population_size=num_paths;
aux_full=0;
tic;
while j<=num_paths
    [path_gen{j},path_events{j}]=createChromosomeRegions3RenewSingle(cluster,states_incr,num_fsm,initial_state_incr,event_list,events_incr,numRobots,numTasks,tasksRegions,initialRegions,numRegions);
...
%             ind=ind+1;
%             [offspring{ind},offspring_event{ind}] = newCrossover3(3,offspring_event{ind-1},states_incr,num_fsm,initial_state_incr,event_list,events_incr,initialRegions);
                                ind=ind+1;
                                [offspring{ind},offspring_event{ind}] = newCrossover3(6,offspring_event{ind-1},states_incr,num_fsm,initial_state_incr,event_list,events_incr,initialRegions);
            %                 end
...
%             ind=ind+1;
            ...
            [offspring{ind},offspring_event{ind}] = newCrossover3(3,offspring_event{ind-1},states_incr,num_fsm,initial_state_incr,event_list,events_incr,initialRegions);
%             ind=ind+1;
%             [offspring{ind},offspring_event{ind}] = newCrossover3(1,offspring_event{ind-2},states_incr,num_fsm,initial_state_incr,event_list,events_incr,initialRegions);
            %             end
            ind=ind+1;
...
% %             ju=1;
% %             
%             [offspring{ind},offspring_event{ind}] =createChromosomeRegions3RenewSingle(cluster,states_incr,num_fsm,initial_state_incr,event_list,events_incr,numRobots,numTasks,tasksRegions,initialRegions,numRegions);
            ind=ind+1;
            [offspring{ind},offspring_event{ind}] = newCrossover3(3,offspring_event{mutation_idx(i)},states_incr,num_fsm,initial_state_incr,event_list,events_incr,initialRegions);
            ind=ind+1;
%             [offspring{ind},offspring_event{ind}] = changeEntryEventsTrunck(offspring{mutation_idx(i)},offspring_event{mutation_idx(i)},states_incr,num_fsm,initial_state_incr,event_list,events_incr);
%             ind=ind+1;
            [offspring{ind},offspring_event{ind}] = newCrossover3(6,offspring_event{mutation_idx(i)},states_incr,num_fsm,initial_state_incr,event_list,events_incr,initialRegions);
            ind=ind+1;
            [offspring{ind},offspring_event{ind}] = newCrossover3(1,offspring_event{mutation_idx(i)},states_incr,num_fsm,initial_state_incr,event_list,events_incr,initialRegions);
%             ind=ind+1;
        end
    else
        for i=1:length(mutation_idx)
