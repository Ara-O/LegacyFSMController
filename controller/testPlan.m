function [finalPlan, PlanCost] = testPlan(plan,cur_state,states,event_list,num_fsms,numRobots,start_loc,tasks,regions,numRegions,numTasks,abstracted,start_events_only,robotTask,events)
...
idleCostFinal=zeros(numTasks,1);
finalTransCost=zeros(numTasks,numRobots);
for i_r = 1:numRobots
...
while ~detectEndState(cur_state,states,num_fsms)
    ...
    if i<=length(plan)
        desired_event=plan{i};
        [aux_index,possible_transitions]=findDesiredTaskEvents(possible_transitions,desired_event);
    end
    
    %     if aux_index ~= 0
    %         i=i+1;
    %         next_event=possible_transitions{i,1};
    %
    ...
    %
    %
    %         full_cost(event_bot)=full_cost(event_bot)+transition_cost;
    %             currBotLoad = full_cost(event_bot);
    %             % Adds Idle cost only to event owner bot
    %             idle_cost = max(0,max(full_cost(:)) - currBotLoad);
    %             % Bot next load = current load + idle cost
    %             full_cost(event_bot) = full_cost(event_bot) + idle_cost;
    %
    ...
    %         end
    %
    %         robotLocation{indj,event_bot} = [new_X new_Y]; %cur_state vs 1
    %
    %     else
    ...
    idle_cost=zeros(num_transitions(1),1);
    ...
    for kk = 1:num_transitions(1) % iterate through the number of transitions
        ...
        % Adds Idle cost only to event owner bot
        idle_cost(kk) = max(0,max(trans_cost(kk,:)) - currBotLoad);
        % Bot next load = current load + idle cost
        trans_cost(kk,event_bot) = trans_cost(kk,event_bot) + idle_cost(kk);
...
%     finalPlan{indi}=possible_transitions{costs_index(index_trans)};
    ...
    full_cost(event_bot)=trans_cost(costs_index(index_trans),event_bot);%%full_cost(event_bot,1)+J_trans(costs_index(index_trans));
    idleCostFinal(indi)=idle_cost(costs_index(index_trans));
    finalTransCost(indi,event_bot)=J_trans(costs_index(index_trans));
    if i<=length(plan)
        if strcmp(next_event,plan{i})
            i=i+1;
        end
    end
end
