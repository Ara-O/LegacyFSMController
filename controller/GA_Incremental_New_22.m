...
[path_abs]=popAbsCluster(20,events,numTasks,numRobots,4);
...
%             if (change_value/12)<1 && aux_same_result<10
    ...
    if aux_same_result>30
        for i=1:length(mutation_idx)
            ...
            ind=ind+1;
            [offspring{ind},offspring_event{ind}] = changeEntryEventsTrunck(offspring{ind-1},offspring_event{ind-1},states_incr,num_fsm,initial_state_incr,event_list,events_incr);
            %[offspring{ind},offspring_event{ind}] = changeEntryEventsTrunck(offspring{ind},offspring_event{ind},states_incr,num_fsm,initial_state_incr,event_list,events_incr);
            ind=ind+1;
            
        end
    else
        for i=1:length(mutation_idx)
            [offspring{ind},offspring_event{ind}] = newCrossover(1,offspring_event{mutation_idx(i)},states_incr,num_fsm,initial_state_incr,event_list,events_incr);
            ind=ind+1;
            [offspring{ind},offspring_event{ind}] = changeEntryEventsTrunck(offspring{mutation_idx(i)},offspring_event{mutation_idx(i)},states_incr,num_fsm,initial_state_incr,event_list,events_incr);
            ind=ind+1;
...
% else
%     limit_half=ceil(3*limit/4);
%     diff=limit-limit_half;
%     for i=1:limit_half
%         path_gen{i}=old_path{idx_fit_aux_2{num_rep}(i)};
%         path_events{i}=old_events{idx_fit_aux_2{num_rep}(i)};
%         cost(1,i)=cost_aux(idx_fit_aux_2{num_rep}(i));
...
%             break;
%         end
%         path_gen{end+1}=old_path{idx_fit_aux_2{num_rep}(i+5)};
%         path_events{end+1}=old_events{idx_fit_aux_2{num_rep}(i+5)};
%         cost(1,end+1)=cost_aux(idx_fit_aux_2{num_rep}(i+5));
%         cost_trans{num_rep}{end+1}=cost_trans_aux{num_rep}{idx_fit_aux_2{num_rep}(i+5)};
%         cost_extra{num_rep}{end+1}=cost_extra_aux{num_rep}{idx_fit_aux_2{num_rep}(i+5)};
%         i=i+1;
% 
%     end
% end
