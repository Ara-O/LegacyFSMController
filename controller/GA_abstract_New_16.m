function [plan,plan_state,plan_cost,num_spoint,initial_plan,num_of_changes,idx_change,trac_plan_cost,num_rep,t_dur,t_pop] = GA_abstract_New_16(population_size,max_rep,mutation_rate,stop_cond,states,start_loc,numTasks,numRobots,numRegions,tasks,regions,robot_tasks)
num_of_changes=0;
abstracted = true;
...
for k=1:sz1(1,1)
    ...
    sz2=size(states{act_state,2}); %number of bifurcation of second state
    num_path2=round(num_path/sz2(1,1));
        for t=1:sz2(1,1)
            for z=1:num_path2
                act_state=1;
                i=1;
                ...
                next_state= states{act_state,2}(t,2); %save the state
                path_events{j}{i}= states{act_state,2}(t,1); %save the tansitions' event
                i=i+1;
                act_state=str2double(next_state);
                aux=1;
                while aux
...
for ik=1:length(old_path_gen)
    ...
    for il=1:b
        ...
        if sum(old_path_gen{ik}==path_gen{il})>=length(old_path_gen{1})
            auxk=0;
%             pause;
            break;
        end
