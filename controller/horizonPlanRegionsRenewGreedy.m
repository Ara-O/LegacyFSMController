function [path_state,path_event,final_region,costTable,robotLocation] = horizonPlanRegionsRenewGreedy(path_event,path_state,costTable,robotLocation,cur_states,final_entry1,states_incr,num_fsm,event_list,events_incr,numRegions,numRobots,initialRegion,numTasks,desired_region1,tasks,regions,states,events)
abstracted = false;
...
for i=1:numRobots
    old_start{i,1}=robotLocation{i}(1);
    old_start{i,2}=robotLocation{i}(2);
end

index=1;
aux_final1=0;
index_st=length(path_state)+1;
index_ev=length(path_event)+1;
% path_state{index_st}=cur_states;
% index_st=index_st+1;
aux_rob=0;
...
while aux_final1==0
    ...
    if aux1~=0
        ...
        path_event{index_ev}=detect_event;
        index_ev=index_ev+1;
        ...
        aux_final1=1;
        aux1=0;
        event_bot = detect_event(1)-96;
        [transition_cost,~,new_X,new_Y] = calcTransitionCostGreedy(detect_event,path_event,index_ev-1,tasks,regions,numRegions,numTasks,abstracted,start_events_only,robotLocation{1,event_bot});
        costTable(event_bot)=costTable(event_bot)+transition_cost;
        
        currBotLoad = costTable(event_bot);
        
        idle_cost = max(0,max(costTable(:)) - currBotLoad);
   
        costTable(event_bot)=costTable(event_bot)+idle_cost;
        ...
        if aux_rob~=0
            ...
            if isempty(pos_transitions_robot)
                ...
                aux_pos_entry=0;
                 [costs,paths,event_path,robotLocation ] = ...
  localDijkstraIncrementalGASingle(pos_transitions_robot,costTable,cur_states,numRobots,states,events,tasks,regions,numRegions,old_start,numTasks,abstracted,start_events_only,old_start);
            ...
            if aux1_region ~=0
                ...
                [costs,paths,event_path,robotLocation ] = ...
                    localDijkstraIncrementalGASingle(robot_entry,costTable,cur_states,numRobots,states,events,tasks,regions,numRegions,old_start,numTasks,abstracted,start_events_only,old_start);
                ind_pot=1;
        ...
        index3=findSameState(path_state,cur_states,num_fsm);
        if isempty(index3)
            path_state{index_st}=cur_states;%saves this state, which will be verified to see if it is the marked state at the next run
            event_bot = detect_event(1)-96;
            [transition_cost,~,new_X,new_Y] = calcTransitionCostGreedy(detect_event,path_event,index_ev-1,tasks,regions,numRegions,numTasks,abstracted,start_events_only,robotLocation{1,event_bot});
            costTable(event_bot)=costTable(event_bot)+transition_cost;
            ...
            old_path_single='';
            old_path_single=path_state;
            path_state{1}='';
            old_event_single='';
            old_event_single=path_event;
            path_event='';
            path_state={old_path_single{1,1:index3}};
            cur_states=old_path_single{1,index3};
            path_event={old_event_single{1:index3-1}};
            index_ev=index3;
            index_st=index3+1;
        end
    end
