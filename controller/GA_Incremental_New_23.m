...
[path_abs]=popAbsCluster(30,events,numTasks,numRobots,4);
num_paths=length(path_abs);
population_size=num_paths;
aux_full=0;
tic;
while j<=num_paths
    [path_gen{j},path_events{j}]=createChromosomeRed(path_abs(j),states_incr,num_fsm,initial_state_incr,event_list,events_incr);
...
%             end
    ...
    if aux_same_result>5
        for i=1:length(mutation_idx)
            ...
            %ind=ind+1;
            [offspring{ind},offspring_event{ind}] = createChromosomeRed(path_j,states_incr,num_fsm,initial_state_incr,event_list,events_incr);
            ind=ind+1;
            [offspring{ind},offspring_event{ind}] = newCrossover(1,offspring_event{ind-1},states_incr,num_fsm,initial_state_incr,event_list,events_incr);
            ind=ind+1;
