function [path_state,path_event,aux_event2,aux_level2] = breathFirstStateTrunck(root,desired_state,states_incr,num_fsm,event_list,events_incr)
aux_event2=0;
aux_level2=0;
...
while ~isempty(list)
    ...
    %robot_entry=findEventsRobot(possible_transitions,robot);%select olny the transitions available for the desired robot
    if ~isempty(possible_transitions)%if there are possible transition
        num_transitions = size(possible_transitions);%check the number of possible transitions       
        for i=1:num_transitions(1)
            detect_event=possible_transitions{i,1}; %check the event
            new_states = updateCurEvent(detect_event,cur_states,events_incr,states_incr,num_fsm);%check to which state that event takes
            aux=checkSameState(desired_state,new_states,num_fsm);
            if aux==0
                if ind_level==7
                    aux_level=1;
                    break;
...
if aux_event==1
        ...
        for j=1:length(list_to_check)
            ...
            for i=1:num_transitions(1)
                ...
                if aux~=0
                    aux_event=1;% mark auxiliary as 1
                    pos_states{end+1}=new_states;% add the state to the list of visited states
                    list_state(end+1)=list_to_check(j);%save the parent of the state
                    list_event{end+1}=detect_event;%save the event that took to this state
                    %check_states{end+1}=new_states;
                    break;%stop the loop since it found the desired event
                end
    ...
    aux_level2=1;
    ind=pop_list(end);%the last event checked at the list before reached level 4
    i=1;
    while list_state(ind) ~=0 %verify if it is not the root
        path(1,i)=ind;%save the state
        ind=list_state(ind);%check the parent of the state
...
%     cur_states=path_state{1}{end};
%     aux=0;
%     aux=detectEndState(cur_states,states_incr,num_fsm);%checkSameState(desired_state,cur_states,num_fsm);
%     while aux==0
%         transition_list = createTransitionList(states_incr,cur_states,num_fsm);%check the possible transitions at this state
%         possible_transitions = findPossibleTransitions(transition_list,event_list);
%         %robot_entry=findEventsRobot(possible_transitions,robot);%select olny the transitions available for the desired robot
%         if ~isempty(possible_transitions)%if there are possible transition
%             num_transitions = size(possible_transitions);%check the number of possible transitions
%             index_2=randi([1 num_transitions(1)]);
%             detect_event=possible_transitions{index_2,1}; %check the event
%             path_event{1}{end+1}=detect_event;
%             cur_states = updateCurEvent(detect_event,cur_states,events_incr,states_incr,num_fsm);%check to which state that event takes
%             index=findSameState(path_state{1},cur_states,num_fsm);
%         if isempty(index)
%             %i=i+1;
%             path_state{1}{end+1}=cur_states;%saves this state, which will be verified to see if it is the marked state at the next run
%         else
%             old_path_single='';
...
%             path_event{1}='';
%             path_state{1}={old_path_single{1,1:index}};
%             cur_states=old_path_single{1,index};
%             path_event{1}={old_event_single{1:index-1}};
%             %i=index;
%         end
%             %path_state{1}{end+1}=cur_states;
%             aux=detectEndState(cur_states,states_incr,num_fsm);%checkSameState(desired_state,cur_states,num_fsm);
%         end
%     end
end
