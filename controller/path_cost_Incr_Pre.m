function [cost,cost_robot,robotLocationFinal] = path_cost_Incr_Pre(cost_robot,path_gen,path_events,start_loc,states,tasks,regions,numRegions,numTasks,abstracted,start_events_only,robot_tasks,numRobots)
...
%     cost_robot=zeros(numRobots,1);
    for j=1:length(path_events)
        ...
        if j==1
            for i_r = 1:numRobots
                robotLocation{j,i_r} = start_loc{1,i_r};%[start_loc(i_r,1) start_loc(i_r,2)]; %cur_state vs 1
%                 cost_robot(i_r)=0;
            end
        end
        ...
        [transition_cost,~,new_X,new_Y] = ...
            calcTransitionCostGAA(next_event,act_state,path_events,j,act_path,states,tasks,0,regions,numRegions,start_loc,...
...
%             cost_robot(w) = cost_robot(w) + idle_cost;%max(0,currBotLoad - cost_robot(w));            
    ...
    cost=max(cost_robot);
    for i_r = 1:numRobots
        robotLocationFinal{1,i_r} = robotLocation{j+1,i_r}; %cur_state vs 1
        %                 cost_robot(i_r)=0;
    end
end