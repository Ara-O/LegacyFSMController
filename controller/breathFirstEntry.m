function [path_state,path_event] = breathFirstEntry(root,robot,final_entry,states_incr,num_fsm,event_list,events_incr)
...
while ~isempty(list)
    ...
    if ~isempty(robot_entry)%if there are possible transition
        aux=checkPosTransEvent(robot_entry,final_entry);%check if the desired event is possible to happen
        if aux==0 %if not
...
j=1;
path_state{1}=root;%save the root
for i=length(path):-1:1
    path_state{end+1}=pos_states{path(i)};%save the visited states
    path_event{j}=list_event{path(i)};%save the events that lead to the visited states
    j=j+1;
end
...
parent=list_state(index);%check its parent. It can be '0', it means we are at the root. Or any other value
%if it is zero, I look for states that has the root as parent, if it is
%something other than zero, I look for the states that has the same parent
...
list_to_check=[];
for i=1:length(list)
    if list_state(list(i))==parent %check at the list of states to be searched, the ones that have same parent as the parent of last state
...
if ~isempty(list_to_check)
    for i=1:length(list_to_check)
        cur_states=pos_states{list_to_check(i)};
        transition_list = createTransitionList(states_incr,cur_states,num_fsm);%check the possible transitions at this state
        possible_transitions = findPossibleTransitions(transition_list,event_list);
        robot_entry=findEventsRobot(possible_transitions,robot);%select olny the transitions available for the desired robot
        if ~isempty(robot_entry)%if there are possible transition
            ...
            if aux ~=0
                ...
                new_states = updateCurEvent(detect_event,cur_states,events_incr,states_incr,num_fsm);%check to which state the event takes to
                pos_states{end+1}=new_states;% add the state to the list of visited states
                list_state(end+1)=list_to_check(i);%save the parent of the state
                list_event{end+1}=detect_event;%save the event that took to this state
            end
        end
    end
