function [costs,path_state,path_events] = ...
  robotDijkstraGreedyNew2(cur_state,numRobots,states,events,tasks,regions,numRegions,start_loc,numTasks,ABSTRACTED,start_events_only,robot_tasks)
...
while ~detectEndState(cur_state,states,num_fsms)
    ...
    if ~isempty(start_transitions)
        ...
        for kk = 1:num_transitions(1) % iterate through the number of transitions
            ...
            J_costs(kk) =transition_cost;%max(trans_cost(kk,:)); %J_costs(event_bot) + transition_cost;
            new_pos(kk,1)=new_X;
            new_pos(kk,2)=new_Y;
        end
