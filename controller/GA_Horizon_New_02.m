...
%Plan
function [all_plan,plan,plan_state,plan_cost,num_spoint,initial_plan,num_of_changes,idx_change,trac_plan_cost,num_rep,t_dur,t_pop] = GA_Horizon_New_02(path_abs,num_paths,max_rep,mutation_rate,stop_cond,states,start_loc,numTasks,numRobots,numRegions,tasks,regions,robot_tasks,states_incr,events_incr,initial_state_incr,initial_state,events)
num_of_changes=0;
abstracted = false;
...
% num_paths=10;
while j<=num_paths
    [path_gen{j},path_events{j}]=createChromosomeRegionsHorizon3(path_abs,states_incr,num_fsm,initial_state_incr,event_list,events_incr,numRobots,numTasks,tasksRegions,initialRegions,numRegions);
...
aux_dup=0;
aux_same_result=0;    
if length(path_events{1})==length(path_abs{1}) || length(path_events{1})==2
    aux_same_result=stop_cond;
    t_dur=0;
end
...
%             total_cost_parents{num_rep}(ind)=cost_parent_1;
            ...
            ind=ind+1;
            [offspring{ind},offspring_event{ind}] = newCrossover2(3,offspring_event{ind-1},states_incr,num_fsm,initial_state_incr,event_list,events_incr,initialRegions);
            %                     ind=ind+1;
            %                     [offspring{ind},offspring_event{ind}] = newCrossover(1,offspring_event{ind-1},states_incr,num_fsm,initial_state_incr,event_list,events_incr);
            %                 end
...
%             total_cost_parents{num_rep}(ind)=cost_parent_1;
            ...
            [offspring{ind},offspring_event{ind}] = newCrossover2(3,offspring_event{ind-1},states_incr,num_fsm,initial_state_incr,event_list,events_incr,initialRegions);
            ind=ind+1;
            [offspring{ind},offspring_event{ind}] = newCrossover2(1,offspring_event{ind-2},states_incr,num_fsm,initial_state_incr,event_list,events_incr,initialRegions);
            %             end
            ind=ind+1;
            ...
            %             if change_value >=1
            ind=ind+1;
            [offspring{ind},offspring_event{ind}] = newCrossover2(1,offspring_event{ind-1},states_incr,num_fsm,initial_state_incr,event_list,events_incr,initialRegions);
            ind=ind+1;
            [offspring{ind},offspring_event{ind}] = newCrossover2(3,offspring_event{ind-2},states_incr,num_fsm,initial_state_incr,event_list,events_incr,initialRegions);
            ind=ind+1;
            %                 [offspring{ind},offspring_event{ind}] = newCrossover(1,offspring_event{ind-1},states_incr,num_fsm,initial_state_incr,event_list,events_incr);
            %             end
...
% % % % %     
% % % % %     for i=1:pop_size
% % % % %         offspring{ind}=path_gen{i};
% % % % %         offspring_event{ind}=path_events{i};
% % % % %         ind=ind+1;
% % % % %         [offspring{ind},offspring_event{ind}] = newCrossover(1,offspring_event{ind-1},states_incr,num_fsm,initial_state_incr,event_list,events_incr);
% % % % %         ind=ind+1;
% % % % %         [offspring{ind},offspring_event{ind}] = newCrossover(3,offspring_event{ind-2},states_incr,num_fsm,initial_state_incr,event_list,events_incr);
% % % % %         ind=ind+1;
% % % % %     end
% % % % %     
...
%         end
    ...
    mutation_idx=mutation_idx_total(1:mutation_idx_max);
        ...
        for i=1:length(mutation_idx)
            [offspring{ind},offspring_event{ind}] = newCrossover2(1,offspring_event{mutation_idx(i)},states_incr,num_fsm,initial_state_incr,event_list,events_incr,initialRegions);
            ind=ind+1;
...
plan=path_events{idx_fit_aux{num_rep}(1)};
plan_state=path_gen{idx_fit_aux{num_rep}(1)};
all_plan=best_event;
plan_cost=cost(1);
end