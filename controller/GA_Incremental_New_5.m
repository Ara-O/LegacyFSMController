%Implementation of more variety on the initial population 
function [all_plan,plan,plan_state,plan_cost,num_spoint,initial_plan,num_of_changes,idx_change,trac_plan_cost,num_rep,t_dur,t_pop] = GA_Incremental_New_5(population_size,max_rep,mutation_rate,stop_cond,states,start_loc,numTasks,numRobots,numRegions,tasks,regions,robot_tasks,states_incr,events_incr,initial_state_incr,initial_state)
num_of_changes=0;
abstracted = false;
...
[path_abs]=generatePopAbstr(initial_state,states,population_size);
...
while j<=num_paths
    ...
    while ~detectEndState(cur_states,states_incr,num_fsm) %check if it didn't reach the final state
        ...
        aux_event=checkEventPath(possible_transitions,path_abs(j),ind_abs);
        if aux_event == 0
        ...
        index=findSameState(path_gen{j},cur_states,num_fsm);
        if isempty(index)
            i=i+1;
            path_gen{j}{i}=cur_states;%saves this state, which will be verified to see if it is the marked state at the next run
        else
            old_path_single='';
            old_path_single=path_gen{j};
            path_gen{j}='';
            old_event_single='';
            old_event_single=path_events{j};
            path_events{j}='';
            path_gen{j}={old_path_single{1,1:index}};
            cur_states=old_path_single{1,index};
            path_events{j}={old_event_single{1:index-1}};
            i=index;
        end
    end
...
%             if new_cost_mut(1,i) > old_cost_mut(1,i)
%                 cost_off(1,mutation_idx(i))=old_cost_mut(1,i);
%                 offspring{mutation_idx(i)}='';
%                 offspring{mutation_idx(i)}=mut_off;
%                 offspring_event{mutation_idx(i)}='';
%                 offspring_event{mutation_idx(i)}=mut_off_events; 
%             end

            t_dur(3,num_rep)=toc(t5);
    end
    
    diff_cost_mutt{num_rep}=old_cost_mut-new_cost_mut;
%     
%     figure;
%     plot(diff_cost_mutt);
