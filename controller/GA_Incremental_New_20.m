...
%             if (change_value/12)<1 && aux_same_result<10
                ...
                if change_value >=1
                    ...
                    %if diff_plan(change_value)>1
                        [offspring{ind},offspring_event{ind}] = newCrossover(3,offspring_event{ind-1},states_incr,num_fsm,initial_state_incr,event_list,events_incr);
                    %else
...
%             if (change_value/12)<1 && aux_same_result<10
    ...
    t3=tic;
    mutation_rate=0;
    if mod(change_value,3)==0 || aux_same_result>10
        mutation_rate=0.3;
        if aux_same_result>25
            mutation_rate=0.5;
        end
    end
    ...
    if aux_same_result > 30
        for i=1:length(mutation_idx)
            ...
            [offspring{ind},offspring_event{ind}] = newCrossover(8,offspring_event{mutation_idx(i)},states_incr,num_fsm,initial_state_incr,event_list,events_incr);
...
%             [offspring{ind},offspring_event{ind}] = createChromossome(path_j,states_incr,num_fsm,initial_state_incr,event_list,events_incr);
        ...
        for i=1:length(mutation_idx)
            ...
            %offspring{mutation_idx(i)}='';
            mut_off_events='';
            mut_off_events=offspring_event{mutation_idx(i)};%offspring_event{mutation_idx(i)};
            %offspring_event{mutation_idx(i)}='';
            ...
            path_abs=stripEventPathMod(mut_off_events);
            mut_abs=stripEventPathMod({mut_off_events{1:idx_mut-2}});
            ind_abs=length(mut_abs)+1;
            ...
            while ~detectEndState(cur_states,states_incr,num_fsm) %check if it didn't reach the final state
                ...
                [aux_event]=checkEventPathMod2(possible_transitions,path_abs,ind_abs);
                if aux_event == 0
                ...
                if isempty(index)
                    ...
                    kj=kj+1;%saves this state, which will be verified to see if it is the marked state at the next run
                else
                    old_path_single='';
