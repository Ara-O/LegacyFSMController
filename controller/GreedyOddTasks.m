function [path_events,cost_final] = GreedyOddTasks(cur_state,numRobots,states,events,tasks,regions,numRegions,start_loc,numTasks,ABSTRACTED,start_events_only)
...
%     num_trans=size(transition_list);
    ...
    for kk = 1:num_transitions(1) % iterate through the number of transitions
        ...
        J_costs(kk) =max(trans_cost(kk,:))-max(full_cost(1,:)); %J_costs(event_bot) + transition_cost;
        new_pos(kk,1)=new_X;
        new_pos(kk,2)=new_Y;
    ...
    desired_value=cmin+alpha*(cmax-cmin);
    
    costs_index=find(J_costs<=desired_value);
...
%     desiredRegion(event_bot,1)=tasksRegions(str2double(possible_transitions{costs_index(index_trans)}(2:end-1)));
    ...
    if length(path_events)==numTasks/2
        finish=1;
    end
end
