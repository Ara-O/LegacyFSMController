function [path,path_events,cost]= createChromosomeGreedy(path_abs,states_incr,num_fsm,initial_state_incr,event_list,events_incr,numRobots,numTasks,taskRegions,initialRegions,numRegions,start_loc,tasks,regions,states,events)
i=1;
abstracted = false;
start_events_only = false;
costTable = zeros(1,numRobots);
for i_r = 1:numRobots
...
prev_state=state_i-1;
prev_event=event_i-1;
while aux_event~=0
    idx_pot=aux_event;
...
% [cost,costTable,robotLocation] = path_cost_Incr_Pre(costTable,path{prev_state:state_i-1},path_events{prev_event:event_i-1},robotLocation,states,tasks,regions,numRegions,numTasks,abstracted,start_events_only,robot_tasks,numRobots);
...
if i-1-aux_finish==1
    ...
    [path,path_events,initialRegions,costTable,robotLocation] = horizonPlanRegionsRenewGreedy(path_events,path,costTable,robotLocation,cur_states,final_event,states_incr,num_fsm,event_list,events_incr,numRegions,numRobots,initialRegions,numTasks,desired_region,tasks,regions,states,events);
...
elseif i-1-aux_finish==2
    ...
    [path,path_events,initialRegions,aux_final1,aux_final2,costTable,robotLocation] = horizonPlanRegionsParallel3RenewSingleGreedy(path_event,path_state,costTable,robotLocation,cur_states,final_event1,final_event2,states_incr,num_fsm,event_list,events_incr,numRegions,numRobots,initialRegions,numTasks,des_region1,des_region2,robot1,robot2);
%     indPlan=randi([1 length(path_event_middle)]);
%     path_events={path_events{1:end},path_event_middle{indPlan}{1:end}};
%     path={path{1:end-1},path_state_middle{indPlan}{1:end}};
    state_i=length(path)+1;
    ...
    cur_states=path{end};
    detect_event=path_events{end};
    aux_finish=aux_finish+aux_final1+aux_final2;

     if ~aux_final1
        finish_event{end+1}=final_event1;
        if strcmp(final_event1(1),'a')
            desired_region1(end+1)=des_region1;
            
        elseif strcmp(final_event1(1),'b')
            desired_region2(end+1)=des_region1;
        else
            desired_region3(end+1)=des_region1;
        end
     end
     if ~aux_final2
         finish_event{end+1}=final_event2;
         if strcmp(final_event2(1),'a')
             desired_region1(end+1)=des_region2;
             
         elseif strcmp(final_event2(1),'b')
             desired_region2(end+1)=des_region2;
         else
             desired_region3(end+1)=des_region2;
         end
     end
else
    ...
    [path,path_events,initialRegions,aux_final1,aux_final2,aux_final3,costTable,robotLocation] = horizonPlanRegionsParallel3RobotsRenewSingleGreedy(cur_states,final_event1,final_event2,final_event3,states_incr,num_fsm,event_list,events_incr,numRegions,numRobots,initialRegions,numTasks,des_region1,des_region2,des_region3,robot1,robot2,robot3);
%     indPlan=randi([1 length(path_event_middle)]);
%     path_events={path_events{1:end},path_event_middle{indPlan}{1:end}};
%     path={path{1:end-1},path_state_middle{indPlan}{1:end}};
    state_i=length(path)+1;
    ...
    cur_states=path{end};
    detect_event=path_events{end};
    aux_finish=aux_finish+aux_final1+aux_final2+aux_final3;
    if ~aux_final1
        finish_event{end+1}=final_event1;
        if strcmp(final_event1(1),'a')
            desired_region1(end+1)=des_region1;
    ...
    if ~aux_final3
        finish_event{end+1}=final_event3;
        if strcmp(final_event3(1),'a')
            desired_region1(end+1)=des_region3;
            
        elseif strcmp(final_event3(1),'b')
            desired_region2(end+1)=des_region3;
        else
            desired_region3(end+1)=des_region3;
        end
    end
end
