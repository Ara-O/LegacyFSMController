...
tic;
while j<=num_paths
    [path_gen{j},path_events{j}]=createChromosomeRegions(path_abs(j),states_incr,num_fsm,initial_state_incr,event_list,events_incr,numRobots,numTasks,tasksRegions,initialRegions,numRegions);
...
%         end
    ...
    if aux_same_result>=0
        for i=1:length(mutation_idx)
            ...
            %ind=ind+1;
            [offspring{ind},offspring_event{ind}] =createChromosomeRegions(path_j,states_incr,num_fsm,initial_state_incr,event_list,events_incr,numRobots,numTasks,tasksRegions,initialRegions,numRegions);
            ind=ind+1;
            [offspring{ind},offspring_event{ind}] = newCrossover(3,offspring_event{ind-1},states_incr,num_fsm,initial_state_incr,event_list,events_incr,initialRegions);
            ind=ind+1;
