 function [all_plan,plan,plan_state,plan_cost,num_spoint,initial_plan,num_of_changes,idx_change,trac_plan_cost,num_rep,t_dur,t_pop] = GA_abstract_New_15(population_size,max_rep,mutation_rate,stop_cond,states,start_loc,numTasks,numRobots,numRegions,tasks,regions,robot_tasks)
num_of_changes=0;
abstracted = false;
    ...
    for i=1:length(mutation_idx)
            ...
            if ~isempty(potential_nodes)
                ...
                if mutation_idx(i)> fix(population_size/2)
                    ...
                    %rng('shuffle');
                    idx_mut=randi([(fix((length(mut_off))/2)+1) length(mut_off)]);
                end
            end
