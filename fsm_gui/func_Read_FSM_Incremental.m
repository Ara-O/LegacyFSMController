function [statesArr, eventsArr, eventNames] = func_Read_FSM_Incremental(fn)
...
statesArr = cell(numel(fn), 1);
eventsArr = cell(numel(fn), 1);
...
for i = 1:numel(fn)
    ...
    % trim so the events only appear once
    eventNames = unique(eventList);
    statesArr{i} = states;
    eventsArr{i} = eventNames;
end
