...
for j = 1:size(tasks,1)
    ...
    regions{i,2} = tasks{j,2}-0.25;
    regions{i,3} = tasks{j,3}-0.25;
    regions{i,4} = tasks{j,2}+0.25;
    regions{i,5} = tasks{j,3}+0.25;
        ...
        for letter=1:size(regions,1)
            ...
            if ~isempty(regOrder)
                ...
                if strncmp(regOrder(k), regions{letter,1},5)
                    ...
                    % And put them in their rightful places
                    tasks{k,2} = (x2-x1)*rand(1) + x1;%round((x2-x1)*rand(1) + x1);
                    tasks{k,3} = (y2-y1)*rand(1) + y1;%round((y2-y1)*rand(1) + y1);
                    ...
                    % make the 2x2 regions with names
                    reg{i,1} = regtsk{k};
                    reg{i,2} = tasks{k,2}-0.25;
                    reg{i,3} = tasks{k,3}-0.25;
                    reg{i,4} = tasks{k,2}+0.25;
                    reg{i,5} = tasks{k,3}+0.25;
                ...
                reg{noRegIndex,2} = tasks{k,2}-0.25;
                reg{noRegIndex,3} = tasks{k,3}-0.25;
                reg{noRegIndex,4} = tasks{k,2}+0.25;
                reg{noRegIndex,5} = tasks{k,3}+0.25;
